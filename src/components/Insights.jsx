...
function CostMetricCard({ label, value, sub, color, t }) {
  return (
    <Card t={t} style={{ padding: "16px 18px" }}>
      ...
      <div style={{ fontSize: "24px", fontWeight: "700", color, fontFamily: "'DM Mono', monospace" }}>
        {value}
      </div>
      {sub ? <div style={{ fontSize: "11px", color: t.textMuted, marginTop: "5px" }}>{sub}</div> : null}
    </Card>
  );
}

function CostTrendTable({ items, t, accent }) {
  return (
    <Card t={t} style={{ overflow: "hidden", marginBottom: "20px" }}>
      <div style={{ padding: "14px 16px", borderBottom: `1px solid ${t.border}`, fontSize: "13px", fontWeight: "700", color: t.text }}>
        AI Cost Trend
      ...
      {!items?.length ? (
        <div style={{ padding: "24px 16px", fontSize: "12px", color: t.textMuted }}>
          Cost trend appears after AI stage usage has been recorded.
        </div>
      ) : (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr 80px", padding: "8px 16px",
            fontSize: "10px", fontWeight: "700", color: t.textMuted, textTransform: "uppercase", letterSpacing: "0.07em",
            borderBottom: `1px solid ${t.border}` }}>
            <span>Date</span><span>Cost</span><span>Calls</span>
          </div>
          {items.map((item, index) => (
            <div key={`${item.date}-${index}`} style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr 80px", padding: "11px 16px", borderBottom: `1px solid ${t.borderLight}`, alignItems: "center" }}>
              <div style={{ fontSize: "12px", color: t.text }}>{item.date}</div>
              <div style={{ fontSize: "12px", color: accent, fontFamily: "'DM Mono', monospace" }}>{formatUsd(item.cost_usd)}</div>
              <div style={{ fontSize: "12px", color: t.textSub, fontFamily: "'DM Mono', monospace" }}>{item.count}</div>
            </div>
          ))}
        </>
      )}
...
function formatDurationMs(value) {
  ...
  if (numeric >= 60000) return `${(numeric / 60000).toFixed(numeric >= 300000 ? 0 : 1)} min`;
  if (numeric >= 1000) return `${(numeric / 1000).toFixed(numeric >= 10000 ? 0 : 1)} s`;
  return `${Math.round(numeric)} ms`;
}

function ReliabilityTable({ title, items, t, accent, emptyLabel = "No data yet", columns = ["Label", "Count", "Value"], mapRow }) {
  return (
    <Card t={t} style={{ overflow: "hidden" }}>
      <div style={{ padding: "14px 16px", borderBottom: `1px solid ${t.border}`, fontSize: "13px", fontWeight: "700", color: t.text }}>
        {title}
      ...
      {!items?.length ? (
        ...
        <>
          <div style={{ display: "grid", gridTemplateColumns: "1.7fr 80px 110px", padding: "8px 16px",
            fontSize: "10px", fontWeight: "700", color: t.textMuted, textTransform: "uppercase", letterSpacing: "0.07em",
            borderBottom: `1px solid ${t.border}` }}>
            <span>{columns[0]}</span><span>{columns[1]}</span><span>{columns[2]}</span>
          </div>
          {items.map((item, index) => {
            const row = mapRow ? mapRow(item) : {
              label: item.label,
              count: item.count,
              value: item.value ?? item.rate,
            };
            return (
              <div key={`${row.label}-${index}`} style={{ display: "grid", gridTemplateColumns: "1.7fr 80px 110px", padding: "11px 16px", borderBottom: `1px solid ${t.borderLight}`, alignItems: "center" }}>
                <div style={{ fontSize: "12px", color: t.text, fontWeight: "500" }}>{row.label}</div>
                <div style={{ fontSize: "12px", color: t.textSub, fontFamily: "'DM Mono', monospace" }}>{row.count}</div>
                <div style={{ fontSize: "12px", color: accent, fontFamily: "'DM Mono', monospace" }}>{row.value}</div>
              </div>
            );
          })}
        </>
...
export function Insights({ t, accent }) {
  ...
  const costMetricCards = [
    { label: "AI Cost", value: formatUsd(aiCosts.total_cost_usd || 0), sub: `${periodLabel} total`, color: "#10B981" },
    { label: "Input Tokens", value: (aiCosts.total_input_tokens || 0).toLocaleString(), sub: "Across all AI stages", color: accent },
    { label: "Output Tokens", value: (aiCosts.total_output_tokens || 0).toLocaleString(), sub: "Generated by the model", color: "#06B6D4" },
    { label: "Top Cost Stage", value: aiCosts.by_stage?.[0]?.label || "—", sub: aiCosts.by_stage?.[0] ? formatUsd(aiCosts.by_stage[0].cost_usd) : "No stage cost yet", color: "#F59E0B" },
  ];

  const queueDepthTotal = (reliability.queue_depth_by_job_type || []).reduce((sum, item) => sum + (item.count || 0), 0);
  const reliabilityCards = [
    { label: "Queue Depth", value: queueDepthTotal.toLocaleString(), sub: reliability.queue_depth_by_job_type?.[0]?.label ? `Top type: ${reliability.queue_depth_by_job_type[0].label}` : "No queued jobs", color: accent },
    { label: "Oldest Pending Job", value: formatDurationMs(reliability.oldest_pending_job_age_ms || 0), sub: "Pending and retry queue age", color: "#F59E0B" },
    { label: "Webhook P95 / P99", value: `${formatDurationMs(reliability.webhook_latency?.p95_ms || 0)} / ${formatDurationMs(reliability.webhook_latency?.p99_ms || 0)}`, sub: `${reliability.webhook_latency?.sample_count || 0} runtime samples`, color: "#06B6D4" },
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "20px" }}>
            <RankedMetricList
              ...
              items={(aiCosts.by_stage || []).map((item) => ({
                label: `${item.label} · ${formatUsd(item.cost_usd)}`,
                count: item.count,
                rate: aiCosts.total_cost_usd > 0 ? Number(((item.cost_usd / aiCosts.total_cost_usd) * 100).toFixed(1)) : 0,
              }))}
              accent="#10B981"
              t={t}
              emptyLabel="No AI stage cost data recorded yet."
            />
            <RankedMetricList
              title="Cost by Channel"
              items={(aiCosts.by_channel || []).map((item) => ({
                label: `${item.label} · ${formatUsd(item.cost_usd)}`,
                count: item.count,
                rate: aiCosts.total_cost_usd > 0 ? Number(((item.cost_usd / aiCosts.total_cost_usd) * 100).toFixed(1)) : 0,
              }))}
              accent={accent}
              t={t}
              emptyLabel="No per-channel AI cost data recorded yet."
            />
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "20px" }}>
            <RankedMetricList
              ...
              items={(aiCosts.by_issue_type || []).map((item) => ({
                label: `${item.label} · ${formatUsd(item.cost_usd)}`,
                count: item.count,
                rate: aiCosts.total_cost_usd > 0 ? Number(((item.cost_usd / aiCosts.total_cost_usd) * 100).toFixed(1)) : 0,
              }))}
              accent="#8B5CF6"
              t={t}
              emptyLabel="No issue-type AI cost data recorded yet."
            />
            <RankedMetricList
              title="Cost by Decision Mode"
              items={(aiCosts.by_decision_mode || []).map((item) => ({
                label: `${item.label} · ${formatUsd(item.cost_usd)}`,
                count: item.count,
                rate: aiCosts.total_cost_usd > 0 ? Number(((item.cost_usd / aiCosts.total_cost_usd) * 100).toFixed(1)) : 0,
              }))}
              accent="#F59E0B"
              t={t}
              emptyLabel="No decision-mode AI cost data recorded yet."
            />
          </div>

          <CostTrendTable items={aiCosts.trend || []} t={t} accent={accent} />
        </>
      )}
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "20px" }}>
            <RankedMetricList
              ...
              items={reliability.queue_depth_by_job_type || []}
              accent={accent}
              t={t}
              emptyLabel="No active queued jobs."
            />
            <RankedMetricList
              title="Retry Count by Job Type"
              items={reliability.retry_count_by_job_type || []}
              accent="#F59E0B"
              t={t}
              emptyLabel="No retrying jobs in the current queue."
            />
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "20px" }}>
            <RankedMetricList
              title="Dead-letter Count by Job Type"
              items={reliability.dead_letter_count_by_job_type || []}
              accent="#EF4444"
              t={t}
              emptyLabel="No failed jobs currently sitting in dead-letter state."
            />
            <RankedMetricList
              title="Worker Throughput by Job Type"
              items={reliability.worker_throughput?.by_job_type || []}
              accent="#10B981"
              t={t}
              emptyLabel="No completed jobs recorded in this range."
            />
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "20px" }}>
            <ReliabilityTable
              title="Worker Claim Latency by Job Type"
              items={reliability.worker_claim_latency?.by_job_type || []}
              accent="#06B6D4"
              t={t}
              emptyLabel="Claim latency appears once jobs have been claimed."
              columns={["Job Type", "Claims", "P95 Claim"]}
              mapRow={(item) => ({
                label: item.label,
                count: item.count,
                value: formatDurationMs(item.p95_ms),
              })}
            />
            <ReliabilityTable
              title="Reply Latency by Channel"
              items={reliability.end_to_end_reply_latency?.by_channel || []}
              accent={accent}
              t={t}
              emptyLabel="Reply latency appears after replies are processed."
              columns={["Channel", "Replies", "P95 Reply"]}
              mapRow={(item) => ({
                label: item.label,
                count: item.count,
                value: formatDurationMs(item.p95_ms),
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "20px" }}>
            <RankedMetricList
              title="Fallback Generation by Channel"
              items={reliability.fallback_generation_rate?.by_channel || []}
              accent="#8B5CF6"
              t={t}
              emptyLabel="No generation fallbacks recorded in this range."
            />
            <RankedMetricList
              title="Failure Rate by Stage"
              items={reliability.failure_rate_by_stage || []}
              accent="#FB7185"
              t={t}
